...
Ctrl+R TUI displays installation, server, and model state. Starting a runtime or loading a model is an
explicitly confirmed action. Once the selected model is ready, the user can hand it to the existing
discovery and role-assignment workflow; runtime management never writes topology configuration.
